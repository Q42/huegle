...
var hue = function ($, colors) { 
    ...
    var shortFlashType = 'select',
        ...
        bridgeIP = Settings.bridgeIP, // use your Hue bridge's IP address here
        apiKey = Settings.bridgeKey, // use the API key you've registered with your hue bridge here
        ...
        offState = { on: false/*, transitiontime:0*/ },
        onState = { on: true, transitiontime:0, effect:'none'},
         ...
         */
        putJSON = function(url, callback, data, sync) {
            var options = {
                type: 'PUT',
                url: url,
                success: function(resp) { console.log("result:", url, resp); callback()},
                error: function(resp) { console.log("error:", url, resp); },
                async: !sync,
                dataType: 'json',
        ...
        postJSON = function(url, callback, data, sync) {
            var options = {
                type: 'POST',
                url: url,
                async: !sync,
        ...
        get = function(destination, success) {
            ...
            $.ajax({
                ...
                url: destination,
                dataType: 'json',
                success: function(d) { stateData=d; },
                data: {},
                async: false
            });
        ...
        postSync = function(destination, data, success) {
            var callback = success || null,
                stateData = null;
            callback = null === callback ? apiSuccess : success;
        ...
        del =  function(destination, success) {
            var callback = success || null,
                stateData = null;
            callback = null === callback ? apiSuccess : success;
        ...
        buildGroupURL = function(groupIndex /* {Number} */) {
            return baseApiUrl + '/groups/' + groupIndex;
        },
         ...
         */
        put = function(lampIndex, data, success, sync) {
            var callback = success || null;
            callback = null === callback ? apiSuccess : success;
		    //data.transitiontime=transitionTime;
			//data.effect="none";
            return putJSON(buildStateURL(lampIndex), callback, data, sync);
        },

        post = function(lampIndex, data, success) {
            var callback = success || null;
            callback = null === callback ? apiSuccess : success;
                      data.transitiontime=0;
                    data.effect="none";
            return postJSON(buildStateURL(lampIndex), callback, data);
        },
        ...
        putGroupAction = function(groupIndex /* {Number} */, action /* String */) {
            ...
            return putJSON(buildGroupActionURL(groupIndex), callback, action, true);
        },

        postGroup = function(stateObj) {
            var callback = apiSuccess;
            return postJSON(baseApiUrl + '/groups', callback, stateObj);
        },
        ...
        buildGroupState = function(groupName, lights) {
            var stateObj = { name: groupName, lights:lights };
            return stateObj;
        },
    ...
    return {
         ...
         * @return {Object} JSON object containing lamp state.
         */
        flashAll: function(groupId) {
            //if (!groupId) return putAll(shortFlashState);
            return putGroupAction(groupId, {'alert':'select'});
        ...
        setColorAndState: function(lampIndex /* Number */, color /* String */, on /*boolean */, extraParams /* obj */) {
            var state = buildXYState(colors.getCIEColor(color));
            if (on) {
                state.on = true;
                if (extraParams) {
                    for (var n in extraParams) {
                        state[n] = extraParams[n];
                    }
                }
            } else {
                state = { on: false }; // don't send a color when off
            }
            return put(lampIndex, state);
        },
        /**
         ...
         * @return {Object} JSON object containing lamp state.
         */
        setAllColors: function(color /* String */, groupId /* optional */) {
            var state = buildXYState(colors.getCIEColor(color));
            if (!groupId) groupId=0;
            return putGroupAction(groupId, state);
        },
        /**
         ...
         * @return {Object} JSON object containing lamp state.
         */
        turnOff: function(lampIndex /* Number */, sync) {
            return put(lampIndex, offState, null, sync);
        },
        /** 
         ...
         * @return {Object} JSON object containing lamp state.
         */
        turnOn: function(lampIndex /* Number */, sync) {
            return put(lampIndex, onState, null, sync);
        },
        /** 
         ...
         * @return {Object} JSON object containing lamp state.
         */
        turnOffAll: function(groupId) {
            if (!groupId) groupId=0;
            return putGroupAction(groupId, offState);
        },
        /** 
         ...
         * @return {Object} JSON object containing lamp state.
         */
        turnOnAll: function(groupId) {
            if (!groupId) groupId=0;
            return putGroupAction(groupId, onState);
        },
        /**
         ...
         */
        setAllBrightness: function(brightness /* Number */, transitiontime) {
            var state = buildBrightnessState(brightness);
            if (transitiontime) state.transitiontime = transitiontime;
        ...
        setIpAndApiKey: function(ip, key) {
            ...
            bridgeIP = ip;
            updateURLs();
            console.log("initialized to bridge with ip:", ip);
        },
        /**
        ...
        listAllGroups: function() {
            var resp = get(baseApiUrl + '/groups', function(resp) { console.log("x"); });
            console.log(resp);
            return resp;
        ...
        deleteGroup: function(groupIndex) {
            del(baseApiUrl + '/groups/' + groupIndex);
        },
        /**
         * create group
         */

        createGroup: function(groupName /* string */, lights /* array */) {
            var obj = {name: groupName, lights:lights};

            var resp = postSync(baseApiUrl + '/groups', obj);
            if (resp[0] && resp[0].success) {
                var str = resp[0].success.id;
                if (str.indexOf("/groups/")==0) return str.substring(8);
            }

            console.log("Warning, error when calling createGroup", groupName);
            return null;
            
        },

        groupColorLoop: function(groupIndex) {
            putGroupAction(groupIndex, {effect:"colorloop"});
        }
...
if(typeof(define) !== 'undefined' && typeof(define.amd) !== 'undefined') {
    ...
    window.hue = hue(window.$, window.colors);
}